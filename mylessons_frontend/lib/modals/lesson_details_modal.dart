...
import '../providers/pack_details_provider.dart'; // Import PackDetailsProvider
import 'package:mylessons_frontend/modals/pack_details_modal.dart'; // Import PackDetailsModal
import 'parents_modal.dart'; // Import the ParentsModal
...
class _LessonDetailsModalState extends State<LessonDetailsModal> {
  ...
  Widget build(BuildContext context) {
    // Wrap in a Scaffold so that showSnackBar works.
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (context, snapshot) {
          ...
          print(details); // Debug print

          // Deduplicate parents across packs.
          ...
          final List<dynamic> uniqueParents = uniqueParentsMap.values.toList();
          ...
          if (widget.currentRole == "Parent") {
            ...
            labelsWithAction = ['Is Done', 'Students', 'Subject', 'Extras', 'Instructors', 'School', 'Location'];
            actionIconMapping = {
              'Date': Icons.edit,
              'Time': Icons.edit,
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  // Card to access Pack Details using ChangeNotifierProvider.
                  Card(
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12),
                    ),
                      ...
                      onTap: () async {
                        if (details.containsKey("packs") &&
                            ...
                            builder: (context) => ChangeNotifierProvider(
                              create: (_) {
                                ...
                                provider.initialize(
                                  ...
                                  unschedulableLessons: null, // update if needed
                                );
                                return provider;
                              },
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("No associated pack found.")),
                          );
                        }
                      },
                    ),
                                      ...
                                      children: [
                                        ...
                                        if (withAction)
                                          IconButton(
                                            ...
                                            onPressed: () async {
                                              if (label == "Is Done") {
                                                final result = await toggleLessonCompletion(lessonId);
                                                if (result != null && result.containsKey("status")) {
                                                  ...
                                                  showDialog(
                                                    ...
                                                    builder: (context) {
                                                      return AlertDialog(
                                                        content: const Text("To complete a lesson make sure the schedule has passed"),
                                                        actions: [
                                                          TextButton(
                                                            onPressed: () => Navigator.pop(context),
                                                            child: const Text("OK"),
                                                ...
                                                bool? updated = await showModalBottomSheet<bool>(
                                                  ...
                                                  builder: (context) => SubjectModal(lessonId: lessonId),
                                                );
                                                if (updated == true) {
                                                  await widget.fetchData();
                                                  _refreshLessonDetails();
                                                ...
                                                bool? updated = await showModalBottomSheet<bool>(
                                                  ...
                                                  builder: (context) => LocationModal(lessonId: lessonId),
                                                );
                                                if (updated == true) {
                                                  await widget.fetchData();
                                                  _refreshLessonDetails();
                                                ...
                                                bool? updated = await showModalBottomSheet<bool>(
                                                  ...
                                                  builder: (context) => StudentsModal(lessonId: lessonId),
                                                );
                                                if (updated == true) {
                                                  await widget.fetchData();
                                                  _refreshLessonDetails();
                                                ...
                                                bool? updated = await showModalBottomSheet<bool>(
                                                  ...
                                                  builder: (context) => InstructorsModal(lessonId: lessonId),
                                                );
                                                if (updated == true) {
                                                  await widget.fetchData();
                                                  _refreshLessonDetails();
                  ...
                  const SizedBox(height: 16),
                  // Add the View Parents card if there are any deduplicated parents.
                      ...
                      child: ListTile(
                        ...
                        onTap: () async {
                          await showModalBottomSheet(
                            ...
                            builder: (context) => ParentsModal(parents: uniqueParents),
                          );
                          _refreshLessonDetails();
                        },
                      ),
